  ...
  get_delegators AS (
    ...
    FROM
      (
        ...
        FROM
          wormhole_op_optimism.WToken_evt_DelegateChanged
      ) b
    WHERE
      update_rank = 1
  ),
  get_balances AS (
    ...
    FROM
      (
        ...
        FROM
          wormhole_op_optimism.WToken_evt_Transfer tf
    ...
    FROM
      (
        SELECT DISTINCT
          ...
          get_href (
            'https://optimistic.etherscan.io/address/' || CAST(gb.delegator AS VARCHAR),
