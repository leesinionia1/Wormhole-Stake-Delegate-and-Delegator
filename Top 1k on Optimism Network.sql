WITH
  ...
  unique_delegators AS (
    ...
    FROM filtered_delegators
    GROUP BY blockchain
  ),
  total_w_delegated_all AS (
    SELECT
      SUM(total_w_delegated_ethereum) + SUM(total_w_delegated_base) + SUM(total_w_delegated_optimism) + SUM(total_w_delegated_arbitrum) AS total_w_delegated_all
    FROM (
      SELECT
        SUM(gb.w_delegated) AS total_w_delegated_ethereum
      FROM get_balances gb
  ...
  top_ethereum AS (
    SELECT
      MAX(CASE WHEN rank = 1000 THEN w_delegated END) AS eth_top_1k,
      MAX(CASE WHEN rank = 5000 THEN w_delegated END) AS eth_top_5k,
      MAX(CASE WHEN rank = 10000 THEN w_delegated END) AS eth_top_10k,
      MAX(CASE WHEN rank = 20000 THEN w_delegated END) AS eth_top_20k
    FROM ranked_delegators_filtered
    WHERE blockchain = 'Ethereum'
  ),
  top_base AS (
    SELECT
      MAX(CASE WHEN rank = 1000 THEN w_delegated END) AS base_top_1k,
      MAX(CASE WHEN rank = 5000 THEN w_delegated END) AS base_top_5k,
      MAX(CASE WHEN rank = 10000 THEN w_delegated END) AS base_top_10k,
      MAX(CASE WHEN rank = 20000 THEN w_delegated END) AS base_top_20k
    FROM ranked_delegators_filtered
    WHERE blockchain = 'Base'
  ),
  top_optimism AS (
    SELECT
      MAX(CASE WHEN rank = 1000 THEN w_delegated END) AS optimism_top_1k,
      MAX(CASE WHEN rank = 5000 THEN w_delegated END) AS optimism_top_5k,
      MAX(CASE WHEN rank = 10000 THEN w_delegated END) AS optimism_top_10k,
      MAX(CASE WHEN rank = 20000 THEN w_delegated END) AS optimism_top_20k
    FROM ranked_delegators_filtered
    WHERE blockchain = 'Optimism'
  ),
  top_arbitrum AS (
    SELECT
      MAX(CASE WHEN rank = 1000 THEN w_delegated END) AS arbitrum_top_1k,
      MAX(CASE WHEN rank = 5000 THEN w_delegated END) AS arbitrum_top_5k,
      MAX(CASE WHEN rank = 10000 THEN w_delegated END) AS arbitrum_top_10k,
      MAX(CASE WHEN rank = 20000 THEN w_delegated END) AS arbitrum_top_20k
    FROM ranked_delegators_filtered
    WHERE blockchain = 'Arbitrum'
  ),
  combined_ranked_delegators AS (
    SELECT
      ROW_NUMBER() OVER (ORDER BY rdf.w_delegated DESC, rdf.days_delegated DESC) AS rank,
      rdf.*
    FROM
...
SELECT
  ...
  twda.total_w_delegated_all,
  ud.unique_delegators,
  te.eth_top_1k,
  te.eth_top_5k,
  te.eth_top_10k,
  te.eth_top_20k,
  ...
  tb.base_top_10k,
  tb.base_top_20k,
  topt.optimism_top_1k,
  topt.optimism_top_5k,
  topt.optimism_top_10k,
  topt.optimism_top_20k,
  tar.arbitrum_top_1k,
  tar.arbitrum_top_5k,
  tar.arbitrum_top_10k,
  tar.arbitrum_top_20k
FROM
  ranked_delegators_filtered rdf
LEFT JOIN
  top_counts tc ON rdf.blockchain = tc.blockchain
LEFT JOIN
  unique_delegators ud ON rdf.blockchain = ud.blockchain
CROSS JOIN
